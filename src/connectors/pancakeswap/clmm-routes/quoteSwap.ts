...
async function quoteClmmSwap(
  ...
  try {
    ...
    let trade;
    if (exactIn) {
      // For SELL (exactIn), we know the input amount.
      // Use parseUnits to avoid floating precision issues with large amounts/18 decimals.
      ...
      // For BUY (exactOut), we know the output amount.
