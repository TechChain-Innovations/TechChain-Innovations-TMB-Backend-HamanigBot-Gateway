...
import { executeAmmSwap } from '../../../../src/connectors/uniswap/amm-routes/executeSwap';
import { getUniswapAmmQuote } from '../../../../src/connectors/uniswap/amm-routes/quoteSwap';
...
describe('uniswap amm executeSwap nonce handling', () => {
  ...
  beforeEach(() => {
    ...
    (getNextNonce as jest.Mock).mockResolvedValue(7);
  ...
  it('uses nonce manager and passes nonce to swap', async () => {
    ...
    (getUniswapAmmQuote as jest.Mock).mockResolvedValue({ quote });
    ...
    expect(getNextNonce).toHaveBeenCalledWith(ethereumInstance.provider, walletAddress, 'mainnet');
